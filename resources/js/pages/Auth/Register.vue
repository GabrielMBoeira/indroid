...
<template>
    ...
    <AppLayout background="/images/ai.jpg">
        <div class="mx-auto grid max-w-5xl items-center gap-10 px-4 py-10 lg:grid-cols-2">
            <div>
                <img src="/images/robot-human.jpg" alt="Robô humanóide do inDROID" class="h-[280px] w-full rounded-[2rem] object-cover neon-border" />
                <p class="mt-4 text-slate-200">Cadastre-se, pague a liberação e comece a impressionar. O termo deixa claro: quem responde é você.</p>
            </div>
            <form class="glass-card rounded-3xl p-8" @submit.prevent="submit">
                ...
                <input v-model="form.email" type="email" required class="mb-3 w-full rounded-xl border border-white/15 bg-black/30 px-4 py-3 outline-none focus:border-neon" />
                ...
                <input :value="form.phone" type="tel" required placeholder="(99) 99999.9999" class="mb-3 w-full rounded-xl border border-white/15 bg-black/30 px-4 py-3 outline-none focus:border-neon" @input="onPhone" />
                ...
                <input v-model="form.password" type="password" required class="mb-3 w-full rounded-xl border border-white/15 bg-black/30 px-4 py-3 outline-none focus:border-neon" />
                ...
                <input v-model="form.password_confirmation" type="password" required class="mb-4 w-full rounded-xl border border-white/15 bg-black/30 px-4 py-3 outline-none focus:border-neon" />
        ...
        <div v-if="showHowTo" class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" @click.self="showHowTo = false">
            <div class="max-w-lg overflow-hidden rounded-3xl glass-card">
                <img src="/images/robot-head.jpg" alt="Como jogar com o inDROID" class="h-48 w-full object-cover" />
                <div class="p-6 text-sm leading-6 text-slate-200">
                    ...
                    <p class="mt-3">2) Faça login e digite a resposta! O robô gera uma frase aleatória para você digitar sem que seu amigo perceba.</p>
                    <p class="mt-3">Depois aperte a tecla ponto "." para assumir o controle da pergunta e completar a frase.</p>
                    <button class="mt-5 rounded-full bg-neon px-5 py-2 font-bold text-ink" @click="showHowTo = false">Entendi!</button>
                </div>
            </div>
        </div>
    </AppLayout>
